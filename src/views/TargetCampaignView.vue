<template>
    <div class="p-6 sm:p-8">
      ...
      <section class="bg-white p-6 rounded-lg shadow mb-8">
        ...
        <form @submit.prevent="addTier" class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-4">
          ...
          <button type="submit" class="btn-green sm:col-span-3">Add Tier</button>
        </form>
        <ul class="list-disc list-inside text-sm text-gray-700">
      ...
      <div class="text-right mt-6">
        <button class="btn-green" @click="saveFullCampaign">🎯 Save Full Campaign</button>
      </div>
    </div>
  </template>
  ...
  async function saveFullCampaign() {
    ...
    for (const tier of tierList.value) {
      await apiAddTargetTier({ campaign_id: newCampaign.id, ...tier });
    }
    alert('✅ Full campaign created!');
    campaignForm.value = { name: '', brand: '', start_date: '', end_date: '' };
